...
import { canAccessPath, getRedirectPath, isPublicRoute } from '@/lib/route-config';
...
export async function middleware(request: NextRequest) {
  ...
  if (isPublic) {
    ...
    if (isAuthenticated && user && (pathname === '/login' || pathname === '/signup')) {
      const dashboardUrl = new URL(getRedirectPath(pathname, user.role, isAuthenticated), request.url);
  ...
  if (isProtectedRoute) {
    ...
    if (!canAccessPath(pathname, user?.role, isAuthenticated)) {
      const redirectUrl = new URL(getRedirectPath(pathname, user?.role, isAuthenticated), request.url);
